from src.busca_utils import SearchResult, is_better, sample_gaussian_neighborhood, target_reached
...
def local_random_search(
    ...
    for iteration in range(1, max_iterations + 1):
        candidate = sample_gaussian_neighborhood(x_best, sigma, bounds, rng)
    ...
    return SearchResult(
        algorithm="Local Random Search",
        x_best=x_best,
        f_best=f_best,
        iterations=iteration,
        stopped_by=stopped_by,
        convergence=convergence,
        hyperparameter_name="sigma",
        hyperparameter_value=sigma,
    )
